%%%-------------------------------------------------------------------
%% @doc 通义千问 (Qianwen) API 客户端 — 复用 OpenAI 兼容格式
%%
%% 通义千问 API 兼容 OpenAI Chat Completions 格式,
%% 因此本模块只是 openai_send 的薄封装, 替换为千问的配置即可。
%%
%% 调用方式:
%%   qianwen_send:generate_content(#{prompt => <<"你好"/utf8>>}).
%%   qianwen_send:generate_content(#{prompt => <<"你好"/utf8>>,
%%       response_format => openai_send:build_response_format(Schema)}).
%%
%% 流式:
%%   qianwen_send:stream_content(#{prompt => <<"你好"/utf8>>}, Callback).
%%   qianwen_send:stream_collect(#{prompt => <<"你好"/utf8>>}).
%%
%% 也可以切换模型:
%%   qianwen_send:generate_content(#{prompt => <<"解释量子计算"/utf8>>},
%%       (qianwen_send:default_config())#{model => <<"qwen-max">>}).
%% @end
%%%-------------------------------------------------------------------
