%%%-------------------------------------------------------------------
%% @doc 智谱 GLM API 客户端 — 上层便捷接口 + 测试函数
%%
%% 对标 openai_test.erl, 提供便捷的调用方法。
%%
%% 在 shell 中测试:
%%   1> glm_test:test_chat().
%%   2> glm_test:test_system().
%%   3> glm_test:test_json().
%%   4> glm_test:test_chat_multi().
%%   5> glm_test:test_stream().
%% @end
%%%-------------------------------------------------------------------
...
%% @doc 简单文本对话
%% 示例: glm_test:chat(<<("你好，请介绍一下自己")/utf8>>).
chat(Text) when is_binary(Text) ->
    glm_send:generate_content(#{?OPT_PROMPT => Text}).

%% @doc 带 System Instruction 的文本对话
%% 示例: glm_test:chat(<<"写一首诗"/utf8>>, <<"你是一位唐代诗人"/utf8>>).
chat(Text, SystemInstruction) when is_binary(Text), is_binary(SystemInstruction) ->
    glm_send:generate_content(#{
        ?OPT_PROMPT             => Text,
...
chat_multi(Messages) when is_list(Messages) ->
    glm_send:generate_content(#{?OPT_MESSAGES => Messages}).
...
stream(Text, Callback) when is_binary(Text), is_function(Callback, 1) ->
    glm_send:stream_content(#{?OPT_PROMPT => Text}, Callback).

%% @doc 流式对话 — 收集全部文本
stream_collect(Text) when is_binary(Text) ->
    glm_send:stream_collect(#{?OPT_PROMPT => Text}).

%%====================================================================
%% 测试函数
%%====================================================================

%% @doc 测试1: 基础文本对话  glm_test:test_chat()
test_chat() ->
    chat(<<("你好，请用一句话介绍一下你自己")/utf8>>).

%% @doc 测试2: 带 System Instruction 的对话  glm_test:test_system()
test_system() ->
    chat(
        <<("写一首关于月亮的诗")/utf8>>,
...
%% @doc 测试3: 多轮对话  glm_test:test_chat_multi()
test_chat_multi() ->
    Messages = [
        {user,      <<("你好")/utf8>>},
...
%% @doc 测试4: JSON 结构化输出  glm_test:test_json()
test_json() ->
    Schema = #{
        <<"type">> => <<"object">>,
        <<"properties">> => #{
...
%% @doc 测试5: 流式对话  glm_test:test_stream()
test_stream() ->
...
%% @doc 测试6: 流式收集完整文本  glm_test:test_stream_collect()
test_stream_collect() ->
    stream_collect(<<("用50字介绍智谱AI")/utf8>>).
